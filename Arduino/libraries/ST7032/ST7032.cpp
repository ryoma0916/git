/*
 ...
 ------------------------
  ...
  Modified 2015/09/12 Moo Soft (Akiyosi Yokoyama)
  ESP8266 対応
...
#include "Arduino.h"
#include <Wire.h>
#include <pgmspace.h>        // Modifyed 2015/09/12 Moo Soft
//#include <avr/pgmspace.h>  // Original
...
// 2015/09 Moo Soft 追加
// ============================== //
void ST7032::beginESP(uint8_t cols, uint8_t lines, uint8_t gpiosda, uint8_t gpioscl, uint8_t dotsize) {
  _displayfunction  = LCD_8BITMODE | LCD_1LINE | LCD_5x8DOTS;
  ...
  // Wire.begin();                                                   // Original
  Wire.begin(gpiosda, gpioscl);                                      // Modifyed 2015/09 Moo Soft
  delay(40);                                                         // Wait time >40ms After VDD stable
  normalFunctionSet();                                               // finally, set # lines, font size, etc.
  extendFunctionSet();
  command(LCD_EX_SETBIASOSC | LCD_BIAS_1_5 | LCD_OSC_183HZ);         // 1/5bias, OSC=183Hz@3.0V
  command(LCD_EX_FOLLOWERCONTROL | LCD_FOLLOWER_ON | LCD_RAB_2_00);  // internal follower circuit is turn on
  delay(200);                                                        // Wait time >200ms (for power stable)
